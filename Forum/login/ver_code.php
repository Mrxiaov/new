<?php
    ...
    $bgcolor=imagecolorallocate($image, 80, 82, 82);//设置背景为白色,默认为黑色 
    imagefill($image, 0, 0, $bgcolor);//填充颜色，把所有与（x，y）颜色相同的点都涂成bgcolor 
    $captch_code='';//保存验证码的变量 
    for($i=0;$i<5;$i++){ 
        $fontsize=2000;//字体大小为6px 
        $fontcolor=imagecolorallocate($image, rand(120,220),rand(120,220),rand(120,220)); 
