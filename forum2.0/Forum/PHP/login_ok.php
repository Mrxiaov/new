        ...
        if (isset($username)&&$password!=null) {
            ...
            if($passwords==$res['password']){  //验证密码是否正确
                ...
                if($res['admin']==1){//验证当前登录用户是不是管理员，是管理跳转后台
                    echo "<script>alert('管理员登录成功');window.location.href='../index.html';</script>";
                }else{//不是管理 ，是用户则跳转到论坛主页
                    echo "<script>alert('登录成功');window.location.href='../index.html';</script>";
                    $_SESSION['username'] = $username;
                }
            }else{//密码不对则跳回登录页面
                echo "<script>alert('用户名或密码不正确!');window.location.href='../index.html';</script>";
            }
        }
        }else{ 
